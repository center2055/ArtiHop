...
use super::{AnonymousPathBuilder, ExitPathLength, TorPath};
use crate::experimental_path::{ExperimentalPathMode, experimental_path_mode};
use crate::path::{pick_exit_path, pick_path};
...
impl ExitPathBuilder {
    ...
    /// Try to create an exit path using the currently selected ArtiHop mode.
    #[instrument(skip_all, level = "trace")]
    pub(crate) fn pick_path_for_current_mode<'a, R: Rng, RT: Runtime>(
        &self,
        rng: &mut R,
        netdir: DirInfo<'a>,
        guards: &GuardMgr<RT>,
        ...
        let length = match experimental_path_mode() {
            ExperimentalPathMode::Standard => ExitPathLength::Standard,
            ExperimentalPathMode::Short2 => ExitPathLength::GuardExit,
            ExperimentalPathMode::Short1 => ExitPathLength::SingleRelay,
        };
        pick_exit_path(self, rng, netdir, guards, config, now, length)
    }
