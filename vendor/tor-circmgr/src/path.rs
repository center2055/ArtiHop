...
/// Exit-circuit hop shape requested by an embedding application.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum ExitPathLength {
    /// Use Arti's standard guard, middle, and exit relay path.
    Standard,
    /// Use a guard and exit relay path.
    GuardExit,
    /// Use only the selected exit relay as the circuit path.
    SingleRelay,
}

/// Try to create and return an exit path with a caller-selected hop shape.
#[instrument(skip_all, level = "trace")]
fn pick_exit_path<'a, B: AnonymousPathBuilder, R: Rng, RT: Runtime>(
    builder: &B,
    rng: &mut R,
    netdir: DirInfo<'a>,
    guards: &GuardMgr<RT>,
    config: &PathConfig,
    _now: SystemTime,
    length: ExitPathLength,
) -> Result<(TorPath<'a>, Option<GuardMonitor>, Option<GuardUsable>)> {
    let netdir = match netdir {
        DirInfo::Directory(d) => d,
        _ => {
            return Err(bad_api_usage!(
                "Tried to build an exit path without a network directory"
            )
            .into());
        }
    };
    ...
    if length == ExitPathLength::SingleRelay {
        let (exit, _) = builder.pick_exit(rng, netdir, target_exclusion, &rs_cfg)?;
        let hops = vec![MaybeOwnedRelay::from(exit)];
        ensure_unique_hops(&hops)?;
        tracing::debug!(path_len = 1, "building experimental single-relay exit path");
        return Ok((
            TorPath::new_multihop_from_maybe_owned(hops),
            None,
            None,
        ));
    }
    ...
    if length == ExitPathLength::GuardExit {
        let hops = vec![guard, MaybeOwnedRelay::from(exit)];
        ensure_unique_hops(&hops)?;
        tracing::debug!(path_len = 2, "building experimental guard-exit path");
        return Ok((
            TorPath::new_multihop_from_maybe_owned(hops),
            Some(mon),
            Some(usable),
