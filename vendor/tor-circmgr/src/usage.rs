        ...
        match self {
            ...
            TargetTunnelUsage::Preemptive {
                ...
                let (path, mon, usable) = ExitPathBuilder::from_target_ports(port.iter().copied())
                    ...
                    .pick_path_for_current_mode(rng, netdir, guards, config, now)?;
                let policy = path
                    .exit_policy()
                    .expect("ExitPathBuilder gave us a path without an exit policy?");
                #[cfg(feature = "geoip")]
            ...
            TargetTunnelUsage::Exit {
                ...
                let (path, mon, usable) =
                    builder.pick_path_for_current_mode(rng, netdir, guards, config, now)?;
                let policy = path
                    .exit_policy()
                    .expect("ExitPathBuilder gave us a path without an exit policy?");
